...
def focal_loss_without_sigmoid(inputs: torch.Tensor, targets: torch.Tensor, num_masks: float, alpha: float = 0.25, gamma: float = 2):
    """
    Loss used in RetinaNet for dense detection: https://arxiv.org/abs/1708.02002.
    Args:
        inputs: A float tensor of arbitrary shape.
...
focal_loss_without_sigmoid_jit = torch.jit.script(
    focal_loss_without_sigmoid
)
...
dice_loss_without_sigmoid_jit = torch.jit.script(
    dice_loss_without_sigmoid
)
...
ce_loss_without_sigmoid_jit = torch.jit.script(
    ce_loss_without_sigmoid
)
